...
class FileManager(HandlerBase):
    ...
    def dedup_videos(self, time_range_str: str) -> int:
        ...
        # 处理记录文件
        processed_log = self.config.DEDUP_DIR / 'processed_videos.log'
        
        # 首先尝试按时间范围获取视频
        video_files = self._get_video_files(start_time, end_time)
        
        # 如果时间范围内没有视频，获取所有未处理的视频
        if not video_files:
            logging.info("时间范围内未找到视频，检查未处理的视频...")
            video_files = self._get_latest_unprocessed_videos(processed_log)
        
        if not video_files:
            logging.info("没有需要处理的视频")
            return 0
            
        logging.info(f"找到 {len(video_files)} 个待处理视频")
        
        with ThreadPoolExecutor(max_workers=self.config.MAX_WORKERS) as executor:
            futures = [executor.submit(self._process_video, file) for file in video_files]
            results = [(video_files[i], f.result()) for i, f in enumerate(futures)]
        ...
        total_processed = sum(1 for _, success in results if success)
        logging.info(f"已处理 {total_processed} 个视频")
        return total_processed

    def _get_video_files(self, start_time: datetime, end_time: datetime) -> List[Path]:
        """获取指定时间范围内的视频文件，支持两级目录结构（用户文件夹/时间戳文件夹）"""
        video_files = []
        # 遍历用户文件夹
        for user_folder in self.config.DOWNLOAD_DIR.glob('*'):
            if not user_folder.is_dir():
                continue
            # 检查是否是时间戳文件夹（直接在 DOWNLOAD_DIR 下）
            folder_time = self._parse_folder_time(user_folder.name)
            if folder_time and start_time <= folder_time <= end_time:
                for file in user_folder.glob('*.mp4'):
                    video_files.append(file)
            else:
                # 遍历用户文件夹下的时间戳子文件夹
                for time_folder in user_folder.glob('*'):
                    if not time_folder.is_dir():
                        continue
                    folder_time = self._parse_folder_time(time_folder.name)
                    if folder_time and start_time <= folder_time <= end_time:
                        for file in time_folder.glob('*.mp4'):
                            video_files.append(file)
        return video_files

    def _get_latest_unprocessed_videos(self, processed_log: Path) -> List[Path]:
        """获取所有未处理的最新视频文件"""
        # 读取已处理记录
        processed_files = set()
        ...
        for user_folder in self.config.DOWNLOAD_DIR.glob('*'):
            if not user_folder.is_dir():
                continue
            # 检查是否是时间戳文件夹
            if self._parse_folder_time(user_folder.name):
                for file in user_folder.glob('*.mp4'):
                ...
                for time_folder in user_folder.glob('*'):
                    if not time_folder.is_dir():
                        continue
                    if self._parse_folder_time(time_folder.name):
                        for file in time_folder.glob('*.mp4'):
                            if str(file) not in processed_files:
                                video_files.append(file)
        return video_files
...
class SchedulerManager:
    ...
    def __init__(
            self, config: AppConfig, file_manager: FileManager, flush_handler: FlushHandler,
